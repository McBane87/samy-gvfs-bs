	...
	if [[ ! -f $(OPT_DONE)/$@ ]]; then \
		cd $(OPT_BUILD)/$(PACK_DIR) && \
			...
			./configure \
				...
				--disable-xml-docs \
				--without-x \
				--with-session-socket-dir=$(OPT_TMPDIR)/.gvfs.dbus \
				--with-system-pid-file=$(OPT_TMPDIR)/.gvfs.dbus/dbus.pid \
				--with-system-socket=$(OPT_TMPDIR)/.gvfs.dbus/dbus.sock \
				--with-console-auth-dir=$(OPT_TMPDIR)/console \
				--with-dbus-user=root && \
		touch $(OPT_DONE)/$@ \
	;fi
	
dbus_make: dbus_configure
	if [[ ! -f $(OPT_DONE)/$@ ]]; then \
		cd $(OPT_BUILD)/$(PACK_DIR) && \
			$(PROG_MAKE_T) && \
		touch $(OPT_DONE)/$@ \
...
dbus_distclean: dbus_prepare dbus_clean
	cd $(OPT_BUILD)/$(PACK_DIR) && \
