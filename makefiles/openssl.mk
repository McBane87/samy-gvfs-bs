.PHONY: openssl	
openssl: zlib openssl_make_install

openssl_prepare:
	$(eval PACKAGE := openssl)
...
openssl_download: openssl_prepare
...
openssl_extract: openssl_download
...
openssl_configure: openssl_extract
	if [[ ! -f $(OPT_DONE)/$@ ]]; then \
		cd $(OPT_BUILD)/$(PACK_DIR) && \
			./Configure  \
				--prefix=$(OPT_PREFIX) \
				--openssldir=$(OPT_PREFIX)/etc/ssl \
				shared \
				threads \
				zlib-dynamic \
				--cross-compile-prefix=$(OPT_CROSS)- \
				linux-generic32 \
				$(OPT_CFLAGS) && \
		touch $(OPT_DONE)/$@ \
	;fi
	
openssl_make: openssl_configure
	if [[ ! -f $(OPT_DONE)/$@ ]]; then \
		cd $(OPT_BUILD)/$(PACK_DIR) && \
			$(PROG_MAKE) depend && \
			$(PROG_MAKE) && \
...
openssl_make_install: openssl_make
	if [[ ! -f $(OPT_DONE)/$@ ]]; then \
		cd $(OPT_BUILD)/$(PACK_DIR) && \
			$(PROG_MAKE) \
			INSTALL_PREFIX=$(OPT_INSTALL) \
			install && \
...
openssl_distclean: openssl_prepare openssl_clean
	cd $(OPT_BUILD)/$(PACK_DIR) && \
